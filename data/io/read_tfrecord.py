...
#     if dataset_name not in ['SHIP', 'ship', 'spacenet', 'pascal', 'coco', 'dota', 'vedia']:
#         raise ValueError('dataSet name must be in pascal or coco')
#
#     if is_training:
#         pattern = os.path.join('../data/tfrecord', dataset_name + '_train.*')
#         # pattern = os.path.join('../data/tfrecord', dataset_name + '_single*')
#     else:
...
def next_batch(dataset_name, batch_size, shortside_len, is_training):
    ...
    if is_training:
        ...
        pattern = os.path.join('../data/tfrecord', dataset_name + '_val.*')
